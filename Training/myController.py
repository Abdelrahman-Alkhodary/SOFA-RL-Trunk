...
import torch
from TD3 import TD3
from utils import PerReplayBuffer
import logging
...
class TrunkController(Sofa.Core.Controller):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # HER replay buffer
        self.replay_buffer = PerReplayBuffer(self.state_dim, self.action_dim)

        self.expl_noise = 0.21
    ...
    def onAnimateBeginEvent(self, eventType):
        if self.starting_state:
            ...
            self.action = (self.policy.select_action(self.state) + np.random.normal(0, 1*self.expl_noise, size=self.action_dim)).clip(0,1)
            ...
            self.step_itr += 1
            reward = self.reward_func(self.step_itr)
            ...
            if self.distance() < 1:
                ...
                print(f'Number {self.goal_reached_count} goal is reached at the step: {self.step_itr} of the episode: {self.episode_itr} with total reward {self.episode_reward}')
                self.logger.debug(f'Number {self.goal_reached_count} goal is reached at the step: {self.step_itr} of the episode: {self.episode_itr} with total reward {self.episode_reward}')
                done = True
                self.episode_done = True
                self.step_itr = 0
                if self.goal_reached_count % 5 == 0:
                    self.policy.save('./weights/weights')
            elif self.step_itr >= self.max_step_itr:
                done = 1
            else:
                done = False

            # push the state and the rest into memory   
            self.replay_buffer.add(self.state, self.action, self.next_state, reward, done)
            ...
            if self.step_itr >= self.max_step_itr:
                self.logger.debug(f'Episode {self.episode_itr} is finished with total reward {self.episode_reward}')
                print(f'Episode {self.episode_itr} is finished with total reward {self.episode_reward}')
                self.logger.debug(f'*****************************************************************************')
                self.step_itr = 0
                self.episode_done = True
                if self.episode_itr % 250 == 0:
                    self.expl_noise -= 0.01
                    self.expl_noise = max(0.05, self.expl_noise)
                if self.episode_itr % self.save_weights_episode == 0:
                    self.policy.save('./weights/weights')
                    if self.episode_itr == 20000:
                        print('##########################')
                        print('## EVALUATE YOUR MAODEL ##')
                        print('##########################')    
        else:
    ...
    def reward_func(self, steps):
        ...
        if self.last_distance < 3:
            if self.current_distance < 3:
                added_reward += 10
        
        self.last_distance = self.current_distance

        return added_reward - self.current_distance 
